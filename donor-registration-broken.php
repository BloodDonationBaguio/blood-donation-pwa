...
// Enable error reporting and display errors temporarily for debugging
        ...
        try {
            ...
            if (empty($errors)) {
                try {
                    ...
                    // Get donor ID and commit the main registration immediately
                    $donorId = $pdo->lastInsertId();
                    $pdo->commit(); // Commit the donor registration right away
                    
                    error_log('Donor registration committed successfully for ID: ' . $donorId);
                    ...
                    return; // Exit early if main registration fails
                }
                
                // Registration succeeded - now handle post-registration tasks
                try {
                    // Handle medical screening separately (completely isolated)
                    error_log('Medical screening data for donor ID ' . $donorId . ': ' . json_encode($medical));
                    error_log('Medical screening database insertion temporarily bypassed - data logged for manual review');
                    ...
                    if ($bloodType === 'Unknown') {
                        $message = "
                            ...
                            <ul>
                                ...
                                <li>The screening process takes only 5-10 minutes</li>
