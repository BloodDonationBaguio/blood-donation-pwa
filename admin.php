...
header("Content-Security-Policy: default-src 'self'; script-src 'self' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com 'unsafe-inline'; style-src 'self' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' https://cdnjs.cloudflare.com data:; connect-src 'self' https://cdn.jsdelivr.net https://cdnjs.cloudflare.com; frame-ancestors 'none'; base-uri 'self'; form-action 'self'");
...
$error = '';
$success = '';
$validTabs = ['dashboard', 'donor-list', 'manage-pages', 'update-contact', 'pending-donors', 'donor-details', 'donor-matching', 'audit-log', 'blood-inventory', 'settings', 'help', 'manual-register'];
        ...
        <li class="nav-item">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 p-4 main-content">
                <div class="dashboard-container">
                                ...
                                if (bloodInventoryCtx && <?= !empty($bloodInventory) ? 'true' : 'false' ?>) {
                                    try {
                                        const bloodInventoryChart = new Chart(bloodInventoryCtx.getContext('2d'), {
                                            type: 'doughnut',
                                            data: {
                                                labels: <?= json_encode(array_column(is_array($bloodInventory) ? $bloodInventory : [], 'blood_type')) ?>,
                                                datasets: [{
                                                    data: <?= json_encode(array_column(is_array($bloodInventory) ? $bloodInventory : [], 'count')) ?>,
                                                    backgroundColor: [
                    ...
                    <?php elseif ($activeTab === 'manual-register'): ?>
                        <?php 
                        $mr_success = '';
                        $mr_error = '';
                        ...
                        if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'manual_register') {
                            try {
                                ...
                                $sql = "INSERT INTO {$donorsTable} (full_name, email, phone, blood_type, date_of_birth, gender, weight, last_donation_date, address, city, state, postal_code, country, reference_code, status, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,'active',CURRENT_TIMESTAMP)";
                                $stmt = $pdo->prepare($sql);
                                $stmt->execute([
                                    $data['full_name'],
                                    $email ?: null,
                            ...
                            } catch (Exception $e) { $mr_error = $e->getMessage(); }
                        }
                        ?>
                        ...
                        <div class="card mb-3" id="eligibilityCheckAdmin" style="display: none;">
                            ...
                            <div class="card-body">
                                ...
                                <div class="alert alert-warning" id="recentDonorWarningAdmin" style="display:none;">
                                    <h6 class="mb-1"><i class="fas fa-exclamation-triangle me-2"></i>Not Eligible Yet</h6>
            ...
            if (window.XMLHttpRequest) {
                ...
                OrigXHR.prototype.open = function(method, url){
                    try {
                        var host = new URL(url, window.location.href).hostname;
                        if (blockedHosts[host]) {
                            // Short-circuit send to avoid console/network noise
                            this.addEventListener('readystatechange', function(){}, false);
                            this.send = function(){ /* no-op */ };
                            return; // do not call underlying open
                        }
                    } catch (e) {}
